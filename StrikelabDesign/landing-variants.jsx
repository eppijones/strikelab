// landing-variants.jsx — four hero compositions for the StrikeLab landing.
// Each variant is a single component that fills its container and tells a
// different story. Switched via the Tweaks panel in Strikelab v2.html.

// Keep the brand palette in sync with components.jsx but add a couple of
// landing-only tokens.
const SL = {
  bg:        '#F2EDE0',
  bgWarm:    '#EBE4D2',
  ...
  mute:      TEE.mute,
  hairline:  TEE.hairline,
  hairline2: TEE.hairline2,
  moss:      '#1A3D2E',
  mossSoft:  'rgba(26,61,46,0.10)',
  sun:       TEE.sun,
  amber:     '#C8913A',
  rust:      TEE.rust,
};
...
function Eyebrow({ children, color = SL.moss, dot = SL.sun }) {
  return (
    <div style={{
...
function ChipLabel({ label, sub, accent = SL.moss, style }) {
  return (
    <div style={{
      ...style,
...
function TopographicHole({ animateKey = 0 }) {
...
function VariantSignature({ lang, t }) {
...
function VariantDayInThree({ lang, t, frame, setFrame }) {
...
function useTicker(from, to, duration = 1400, decimals = 0, deps = []) {
...
function NumberTile({ eyebrow, value, unit, footnote, dark, big, accent = SL.moss }) {
  return (
    <div style={{
...
function VariantNumbers({ lang, t, cycleKey }) {
...
function MiniAppIcon({ size = 20, bg = SL.moss, fg = SL.surface }) {
  return (
    <div style={{
...
function VariantConstellation({ lang, t, cycleKey }) {
...
function ReplayingMark({ size, intervalMs = 4400 }) {
  const [k, setK] = React.useState(0);
...
Object.assign(window, {
  SL, Eyebrow, ChipLabel,
  VariantSignature, VariantDayInThree, VariantNumbers, VariantConstellation,
  DAY_FRAMES,
});
